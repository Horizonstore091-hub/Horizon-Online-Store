  ...
  useEffect(() => {
    try {
      ...
      if (uid) {
        ...
        fetch(`/api/wishlist/${uid}`).then(r => r.json()).then(list => {
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16 md:h-20">
          ...
          <div className="flex items-center gap-1">
            ...
            <Link to="/wishlist" className="relative p-2 text-gray-600 dark:text-gray-200 hover:text-midnight-900 dark:hover:text-white transition-colors" title="Wishlist">
              ...
              {wishlistCount > 0 && <span className="absolute -top-0.5 -right-0.5 bg-pink-500 text-white text-[10px] font-bold w-4 h-4 rounded-full flex items-center justify-center">{wishlistCount > 9 ? '9+' : wishlistCount}</span>}
            </Link>
