...
export default function AdminBrands() {
  ...
  const load = () => {
    setLoading(true);
    fetch('/api/admin/brands').then(r => r.json()).then(data => { setBrands(data); setLoading(false); }).catch(() => setLoading(false));
  };
  ...
  const editBrand = (b) => { setForm({ name: b.name, description: b.description || '' }); setEditing(b.id); setShowForm(true); };

  const deleteBrand = async (id) => {
    if (!confirm('Delete this brand?')) return;
    await fetch(`/api/admin/brands/${id}`, { method: 'DELETE' }); load();
  };
    ...
    <div className="flex min-h-screen bg-gray-50 dark:bg-midnight-950">
      ...
      <div className="flex-1 p-8">
        <div className="flex items-center justify-between mb-8">
          <div><h1 className="text-2xl font-bold text-midnight-900 dark:text-white">Brands</h1><p className="text-sm text-gray-500 dark:text-gray-400 mt-1">Manage product brands</p></div>
          <button onClick={() => { setEditing(null); setForm({ name: '', description: '' }); setShowForm(!showForm); }} className="btn-primary text-sm !py-2 !px-4">{showForm ? 'Cancel' : 'Add Brand'}</button>
        </div>
        {showForm && (
          <form onSubmit={handleSubmit} className="admin-card mb-8 animate-slide-down">
            <h2 className="text-lg font-semibold mb-4">{editing ? 'Edit Brand' : 'Add Brand'}</h2>
          ...
          <div className="admin-card overflow-hidden !p-0">
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-gray-100 dark:divide-midnight-800">
                {brands.map(b => (
                  <tr key={b.id} className="hover:bg-gray-50 dark:hover:bg-midnight-800/30 transition-colors">
                    <td className="p-4 text-midnight-900 dark:text-white font-medium">{b.name}</td>
                    <td className="p-4 text-gray-500">{b.slug}</td>
                    <td className="p-4 text-gray-500">{b.description}</td>
                    <td className="p-4 text-right"><button onClick={() => editBrand(b)} className="text-horizon-600 hover:text-horizon-700 dark:text-horizon-400 mr-3">Edit</button><button onClick={() => deleteBrand(b.id)} className="text-red-500 hover:text-red-600">Delete</button></td>
                  </tr>
                ))}
              </tbody>
            </table>
